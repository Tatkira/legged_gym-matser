...
// OSQP-specific utility functions
bool near_zero(OSQPFloat a) { return (a < 0.01 && a > -.01); }

bool near_one(OSQPFloat a) { return near_zero(a - 1); }
...
class ConvexMpc {
 public:
  ...
  virtual ~ConvexMpc() { osqp_cleanup((OSQPSolver*)workspace_); }
...
std::vector<double> ConvexMpc::ComputeContactForces(
  ...
  if (qp_solver_name_ == OSQP) {
    ...
    if (workspace_ == 0) {
      ...
      workspace_ = (OSQPWorkspace*)solver;
      initial_run_ = false;
    } else {
    ...
    if (workspace_->info->status_val == OSQP_SOLVED) {
      solution =
          -Map<const VectorXd>(workspace_->solution->x, workspace_->data->n);
    } else {
